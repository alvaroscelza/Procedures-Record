...
if (termsAndConditionsControl()) {
    if (userIsLoggedIn()) {
        ...
        tryToDisplaySmartyTemplate('Procedures.tpl', $smartyVariables);
    } else {
...
function getSmartyVariablesToAssign() {
    if (termsAndConditionsControl()) {
        ...
        $proceduresForSelectedPerson = obtainAllProceduresFromPerson($personId);
        return array(
            'userProfile' => $userProfile,
            'personId' => $personId,
            'proceduresForSelectedPerson' => $proceduresForSelectedPerson
        );
    } else {
        showTermsAndConditionsControlFailureMessage();
    }
